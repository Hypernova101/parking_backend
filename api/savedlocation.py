...
class SavedLocationAPI:
    ...
    class _CRUD(Resource):
        ...
        def post(self):
            """
            Add new savedlocation(s) to the authenticated user's existing savedlocation.
            """
            current_user = g.current_user
            user = User.query.filter_by(_uid=current_user.uid).first()
            if not user:
            ...
            current_locations = user.savedlocation.split(', ') if user.savedlocation else []
            combined_locations = list(set(current_locations + formatted_new_savedlocation.split(', ')))

            user.savedlocation = ', '.join(combined_locations)
        ...
        def delete(self):
            ...
            savedlocations = user.savedlocation.split(', ') if user.savedlocation else []
